...
const FlickeringGrid: React.FC<FlickeringGridProps> = ({
  ...
  const drawGrid = useCallback(
    (ctx: CanvasRenderingContext2D, width: number, height: number, cols: number, rows: number, squares: Float32Array) => {
  ...
  useEffect(() => {
    ...
    let { width, height, cols, rows, squares } = setupCanvas(canvas);
    ...
    const animate = (time: number) => {
      ...
      drawGrid(ctx, width, height, cols, rows, squares);
      animationFrameId = requestAnimationFrame(animate);
    };

    const handleResize = () => {
      ({ width, height, cols, rows, squares } = setupCanvas(canvas));
    };
